...
void modo_automatico(){
  tempo_decorrido = millis() - tempo_inicializacao;
  if(tempo_decorrido - tempo_inicial > 30000 && quant_mensagem_falha == 0){
  ...
  if (quant_mensagem_falha == 1 || (falha_mqtt == true && (int(round((millis() - tempo_acionado) / 1000)) % 25 == 0 || int((round(millis() - tempo_acionado) / 1000)) % 26 == 0 || int(round((millis() - tempo_acionado) / 1000)) % 27 == 0 || int(round((millis() - tempo_acionado)/ 1000)) % 28 == 0)) || (falha_mqtt == false && (int(round((millis() - tempo_acionado)/ 1000)) % 25 == 0))){
    tempo_acionado = millis();
    client.publish(topico_confirmacao, "B");
    delay(5000);
    quant_mensagem_falha = 0;
    client.loop();
    if(tempo_decorrido - tempo_inicial > 30000){
      //Serial.println("Uma nova mensagem não chegou no tempo esperado.");
      //quant_mensagem_falha += 1;
      //amarelo_piscante(1);
      //}else{
      client.publish(topico_confirmacao, "B");
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect("SemaforB_ESP")) {
...
void setup_wifi() {
  ...
  if (resultado == 0) {
    ...
    WiFi.config(IPAddress(192,168,0,11), IPAddress(192,168,0,1), IPAddress(255,255,252,0));
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (mensagem1.indexOf("irmacaoBV\"") != -1) {
    Serial.println("Encontrado!");
    mensagem1.replace("irmacaoBV", "{\"A\": {\"V");
    mensagem = mensagem1; 
  }
  
  if (mensagem1.indexOf("irmacaoB\"") != -1) {
    Serial.println("Encontrado!");
    // substitui "irmacaoB\"" por {"A": "L"
    mensagem1.replace("irmacaoB\"", "{\"A\": \"L\"");
    mensagem = mensagem1;
  }
  ...
  JsonVariant variant = doc["B"];

  if (variant.is<JsonObject>()) {
    mensagem_recebida = true;

    JsonObject objetoB = variant.as<JsonObject>();

    String chaveInterna = "";
    double valor = 0;

    for (JsonPair kv : objetoB) {
  ...
  if (chaveInterna == "V"){
    String liberacao = "{\"B\": " + String(valor) + "}";
